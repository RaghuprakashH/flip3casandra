...
class FlipkratScrapper:
    ...
    def getAvailableOffer(self):
        ...
        try:
            ...
            list(final_data).append(offer_details)
    ...
    def generatingResponse(self, product_searched, product_name, price, discount_percent, offer_details, EMI, result):
        ...
        try:
            ...
            table_review_age = []
            table_review_age.append(review_age)
            final_data = []
            final_data.append(product_name,offer_details,comments,cust_name,EMI,discount_percent,price,product_searched,rating,table_review_age)
    ...
    def getReviewsToDisplay(self, searchString, expected_review, username, password, review_count):
        ...
        try:
            search = searchString
            #session = CassaDBManagement(username=username, password=password)
            CassaDB = CassaDBManagement(username='username', password='password')
            session1 = CassaDB.getCassaDBClientObject()
            count_record  = CassaDB.iscountDatabase(countrec='countrec')
            print('count_record:',count_record)
                ...
                if review_count <= expected_review:
                    ...
                    if locator.getCustomerName() in self.driver.page_source:
                     ...
                     #   session = CassaDBManagement(username=username, password=password)
                        ...
                        db_search = []
                        db_search1 = []
                        db_search1 = CassaDB.isProductPresent(firstrow1)

                        if firstrow1 > 0:
                            db_search = CassaDB.isProductPresent1(firstrow2)
                            ...
                            if len(ratings) > 0:
                                for i in range(0, len(ratings)):
                                    ...
                                    try:
                                        ...
                                        Cno = Cno + 1
                                        qr1 = 'INSERT INTO ineuron3.review (cno,comment,customer_name,discount_percent,emi_detail,offer_details,price,product_name,product_searched,ratings,review_age) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
                                        batch = BatchStatement()
                                        batch.add(qr1, final_data)
